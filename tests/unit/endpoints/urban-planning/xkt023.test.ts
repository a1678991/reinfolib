...
import {
  ...
  responseSchema,
  call,
} from "../../../../src/endpoints/urban-planning/xkt023.js";

const geoFixture = JSON.parse(readFileSync(resolve("tests/fixtures/xkt023.json"), "utf8"));
const pbfFixture = readFileSync(resolve("tests/fixtures/xkt023.pbf"));

describe("XKT023 params schema", () => {
...
describe("XKT023 response schema", () => {
...
describe("ReinfolibClient.urbanPlanning.districtPlans", () => {
  it("is wired (GeoJSON)", async () => {
    const fetchFn = vi.fn(async () => new Response(JSON.stringify(geoFixture), { status: 200 }));
    const client = new ReinfolibClient({ apiKey: "k", fetch: fetchFn, rateLimit: false });
    const res = await client.urbanPlanning.districtPlans({ z: 14, x: 14551, y: 6451 });
    expect(res.ok).toBe(true);
  });
  ...
  it("is wired (PBF)", async () => {
    const fetchFn = vi.fn(async () => new Response(pbfFixture, { status: 200 }));
    const client = new ReinfolibClient({ apiKey: "k", fetch: fetchFn, rateLimit: false });
    const res = await client.urbanPlanning.districtPlans(
      { z: 14, x: 14551, y: 6451 },
      { format: "pbf" },
    );
    expect(res.ok).toBe(true);
