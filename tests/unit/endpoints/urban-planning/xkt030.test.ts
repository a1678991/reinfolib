...
import {
  ...
  responseSchema,
  call,
} from "../../../../src/endpoints/urban-planning/xkt030.js";

const geoFixture = JSON.parse(readFileSync(resolve("tests/fixtures/xkt030.json"), "utf8"));
const pbfFixture = readFileSync(resolve("tests/fixtures/xkt030.pbf"));

describe("XKT030 params schema", () => {
  it("accepts a valid request", () => {
    expect(paramsSchema.safeParse({ z: 11, x: 1828, y: 751 }).success).toBe(true);
  });
...
describe("XKT030 response schema", () => {
...
describe("XKT030 call() — GeoJSON", () => {
  it("returns ok with parsed FeatureCollection", async () => {
    ...
    const res = await call(client, { z: 11, x: 1828, y: 751 });
...
describe("ReinfolibClient.urbanPlanning.plannedRoads", () => {
