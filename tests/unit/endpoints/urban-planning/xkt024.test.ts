...
import {
  ...
  responseSchema,
  call,
} from "../../../../src/endpoints/urban-planning/xkt024.js";

const geoFixture = JSON.parse(readFileSync(resolve("tests/fixtures/xkt024.json"), "utf8"));
const pbfFixture = readFileSync(resolve("tests/fixtures/xkt024.pbf"));

describe("XKT024 params schema", () => {
  it("accepts a valid request", () => {
    expect(paramsSchema.safeParse({ z: 14, x: 14551, y: 6451 }).success).toBe(true);
  });
...
describe("XKT024 response schema", () => {
...
describe("XKT024 call() — GeoJSON", () => {
  it("returns ok with parsed FeatureCollection", async () => {
    ...
    const res = await call(client, { z: 14, x: 14551, y: 6451 });
...
describe("ReinfolibClient.urbanPlanning.highUseDistricts", () => {
  it("is wired (GeoJSON)", async () => {
    const fetchFn = vi.fn(async () => new Response(JSON.stringify(geoFixture), { status: 200 }));
    const client = new ReinfolibClient({ apiKey: "k", fetch: fetchFn, rateLimit: false });
    const res = await client.urbanPlanning.highUseDistricts({ z: 14, x: 14551, y: 6451 });
    expect(res.ok).toBe(true);
  });
  ...
  it("is wired (PBF)", async () => {
    const fetchFn = vi.fn(async () => new Response(pbfFixture, { status: 200 }));
    const client = new ReinfolibClient({ apiKey: "k", fetch: fetchFn, rateLimit: false });
    const res = await client.urbanPlanning.highUseDistricts(
      { z: 14, x: 14551, y: 6451 },
      { format: "pbf" },
    );
    expect(res.ok).toBe(true);
